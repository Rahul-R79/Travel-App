...
export default function Login() {
    ...
    const handleSubmit = (e) =>{
        ...
        .then((response) =>{
            ...
            localStorage.setItem("user_data", JSON.stringify(data)) //Storing token in local storage>>>>
            UpdateUserData({type: "LOGIN", payload: data}) //Updating userContext on login and signup page
            homepage('/') //Redirecting after login>>>>
        })
        .catch(error =>{
  ...
  return (
        <>
        <Helmet>
            <title>Login Page ~ Travel | App</title>
        </Helmet>
        <Container>
            <LeftContainer>
                <HeaderContainer>
            ...
            <RightContainer>
                <LoginContainer>
                    ...
                    <Form onSubmit={handleSubmit}>
                        <InputContainer>
                            <TextInput type="email" placeholder="Email" onChange={(e) =>setUsername(e.target.value)} value={username}/>
                        </InputContainer>
                        <InputContainer>
